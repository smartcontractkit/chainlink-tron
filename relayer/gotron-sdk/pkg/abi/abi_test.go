...
func TestABIParamArray(t *testing.T) {
	...
	`))
	b, err := GetPaddedParam([]any{param})
	require.Nil(t, err)
...
func TestABIParamArrayUint256(t *testing.T) {
	b, err := GetPaddedParam([]any{Param{"uint256[2]": []string{"100000000000000000000", "200000000000000000000"}}})
	require.Nil(t, err)
	assert.Len(t, b, 64, fmt.Sprintf("Wrong length %d/%d", len(b), 64))
...
func TestGetPaddedParamInterfaceArray(t *testing.T) {
	...
	result, err := GetPaddedParam([]any{param})
