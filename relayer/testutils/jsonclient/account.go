...
type GetAccountResponse struct {
	...
	//FrozenV2                                     []FreezeV2       `json:"frozenV2"`                                          // []	In Stake 2.0, the total amount of TRX staked to obtain various types of resources does not include the delegated TRX
	//UnfrozenV2                                   []UnFreezeV2     `json:"unfrozenV2"`                                        // []	In Stake 2.0, each unstaking information. One of the unstaking information contains three fields: type: resource type; unfreeze_amount: the amount of unstaked TRX; unfreeze_expire_time: the start time stamp when the unstaked TRX can be withdrawn, in ms.
	...
	FreeAssetNetUsagev2   map[string]int64 `json:"free_asset_net_usageV2"`   // <string, int64>	The amount of trc10's free bandwidth used by this account
...
func (tc *TronJsonClient) GetAccount() (*GetAccountResponse, error) {
	getAccountEndpoint := "/wallet/getaccount"
	getAccountResponse := GetAccountResponse{}

	_, _, err := tc.post(tc.baseURL+getAccountEndpoint, gaRequest, &getAccountResponse)
	if err != nil {
