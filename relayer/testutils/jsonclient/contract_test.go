...
func TestEstimateEnergy(t *testing.T) {
  ...
  "energy_required": 1082
	...
	code := http.StatusOK
	jsonclient := NewTronJsonClient("baseurl", NewMockJsonClient(code, response, nil))
...
func TestEstimateEnergyFail(t *testing.T) {
	jsonclient := NewTronJsonClient("http://endpoint", NewMockJsonClient(500, "", fmt.Errorf("request failed")))
	r := require.New(t)
...
func TestContractGet(t *testing.T) {
  ...
  "code_hash": "5933c5f6804befa730c18e6bf1b14393d9e062b466c2a772f81a6bfa932a8d46"
	...
	code := http.StatusOK
	jsonclient := NewTronJsonClient("baseurl", NewMockJsonClient(code, contractjson, nil))

	a := assert.New(t)
	r := require.New(t)

	contract, err := jsonclient.GetContract("someaddress")
...
func TestContractDeploy(t *testing.T) {
  ...
  "visible": true
	...
	transaction, err := jsonclient.DeployContract(&deployRequest)
